  ...
  return (
    <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
        <div className='w-full max-w-md'>
            ...
            <div className='rounded-lg h-96 flex items-center justify-center relative' style={{backgroundColor: mode === 'text' ? background : 'transparent'}}>
                {
                    mode === 'text' && (
                        <textarea onChange={(e)=>setText(e.target.value)} value={text} className='bg-transparent text-white w-full h-full p-6 text-lg resize-none focus:outline-none' placeholder="What's on your mind?"/>
                    )
                }
                {
            ...
            <div className='flex mt-4 gap-2'>
                {bgColors.map((color)=> (
                    <button key={color} style={{backgroundColor: color}} onClick={()=> setBackground(color)} className='w-6 h-6 rounded-full ring cursor-pointer' />
                ))}
            </div>
