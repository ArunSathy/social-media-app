...
import { MapPin, Pencil, User, AtSign, SquareUserRound } from 'lucide-react';
            ...
            <form className='space-y-4' onSubmit={handleSaveProfile}>
              ...
              <div className='flex flex-col items-start gap-3'>
                ...
                <label htmlFor="profile_picture" className='block text-sm font-medium text-gray-700 mb-1'>
                  Profile Picture
                  <input hidden onChange={(e)=>setEditForm({...editForm, profile_picture: e.target.files[0]})} type="file" accept='image/*' id='profile_picture' className='w-full p-3 border border-gray-200 rounded-lg' />
              ...
              <div className='flex flex-col items-start gap-3'> 
                <label htmlFor="cover_photo" className='block text-sm font-medium text-gray-700 mb-1'>
                  Cover Photo
                  <input hidden onChange={(e)=>setEditForm({...editForm, cover_photo: e.target.files[0]})} type="file" accept='image/*' id='cover_photo' className='w-full p-3 border border-gray-200 rounded-lg' />
                  
                  <div className='group/cover relative'>
                    <img className='w-80 h-40 rounded-lg bg-linear-to-r from-indigo-200 via-purple-200 to-pink-200 object-cover mt-2' src={editForm.cover_photo ? URL.createObjectURL(editForm.cover_photo) : user.cover_photo} alt="" />
              ...
              <div>
                ...
                <textarea rows={3} className='w-full p-3 border border-gray-200 rounded-lg' placeholder='Enter a Bio' onChange={(e)=>setEditForm({...editForm,bio: e.target.value})} value={editForm.bio} />
              </div>
